...
//
// The key is ACCOUNT-level, so it is the same one buffy and the coffee app ship
// with; no chawan-specific key exists or is needed. Ported from
// ~/Desktop/CODE/scripts/asc-api.mjs — kept byte-compatible on purpose so a fix
// in one repo can be copied across without a diff to reconcile.
//
// Usage:
//   node scripts/asc-api.mjs GET  /v1/apps/6788678149/appStoreVersions
